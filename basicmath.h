...
public:
    ...
    myvector operator/ (const double scale) const {
        assert(scale!=0);
        return myvector (ijk_[0]/scale, ijk_[1]/scale, ijk_[2]/scale);
    }
    ...
    void bisector(const myvector va, const myvector vb)
    {
        myvector trs = va;
        trs+=vb;
        double vec_value = trs.valueOf();
        ijk_[0] = trs.ijk_[0]/vec_value;
        ijk_[1] = trs.ijk_[1]/vec_value;
        ijk_[2] = trs.ijk_[2]/vec_value;
    }
    void componentWiseMultiple(const myvector va, const myvector vb)
    {
        ijk_[0] = va.ijk_[0]*vb.ijk_[0];
        ijk_[1] = va.ijk_[1]*vb.ijk_[1];
        ijk_[2] = va.ijk_[2]*vb.ijk_[2];
    }
...
public:
    ...
    myvector operator- (const mypoint &p) const {
        ...
        //
    mypoint operator+ (const myvector &p) const {
        return mypoint (
                         xyz_[0] + p[0],
                         xyz_[1] + p[1],
                         xyz_[2] + p[2]);
    ...
    double operator* (const myvector &p) const{
        return xyz_[0]*p[0]+xyz_[1]*p[1]+xyz_[2]*p[2];
    }
